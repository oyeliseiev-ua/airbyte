...
public class SinglestoreSourceOperations extends
    AbstractJdbcCompatibleSourceOperations<SinglestoreType> implements
    SourceOperations<ResultSet, SinglestoreType> {

  private static final Set<SinglestoreType> ALLOWED_CURSOR_TYPES = Set.of(TINYINT, TINYINT_UNSIGNED,
      SMALLINT, SMALLINT_UNSIGNED, MEDIUMINT, MEDIUMINT_UNSIGNED, INT, INT_UNSIGNED, BIGINT,
      BIGINT_UNSIGNED, FLOAT, FLOAT_UNSIGNED, DOUBLE, DOUBLE_UNSIGNED, DECIMAL, DECIMAL_UNSIGNED,
      DATE, DATETIME, TIMESTAMP, TIME, YEAR, VARCHAR, TINYTEXT, TEXT, MEDIUMTEXT, LONGTEXT);

  private static final Logger LOGGER = LoggerFactory.getLogger(SinglestoreSourceOperations.class);

  @Override
  public void copyToJsonField(@NotNull ResultSet resultSet, int colIndex, @NotNull ObjectNode json)
      throws SQLException {
    ...
    switch (columnType) {
      case BIT, TINYBLOB, BLOB, MEDIUMBLOB, LONGBLOB, BINARY, VARBINARY ->
      ...
      case SMALLINT, SMALLINT_UNSIGNED, MEDIUMINT, MEDIUMINT_UNSIGNED, INT ->
          putInteger(json, columnName, resultSet, colIndex);
      case INT_UNSIGNED, BIGINT, BIGINT_UNSIGNED ->
  ...
  @Override
  public void setCursorField(@NotNull PreparedStatement preparedStatement, int parameterIndex,
      final SinglestoreType cursorFieldType, @NotNull String value) throws SQLException {
    ...
    switch (cursorFieldType) {
      ...
      case YEAR, TINYINT, TINYINT_UNSIGNED, SMALLINT, SMALLINT_UNSIGNED, MEDIUMINT, MEDIUMINT_UNSIGNED ->
          setInteger(preparedStatement, parameterIndex, value);
      case INT, INT_UNSIGNED, BIGINT, BIGINT_UNSIGNED ->
          setBigInteger(preparedStatement, parameterIndex, value);
      case FLOAT, FLOAT_UNSIGNED, DOUBLE, DOUBLE_UNSIGNED ->
  ...
  public SinglestoreType getDatabaseFieldType(@NotNull JsonNode field) {
    try {
      ...
      LOGGER.warn(String.format(
          ...
          field.get(JdbcConstants.INTERNAL_COLUMN_NAME),
          field.get(JdbcConstants.INTERNAL_SCHEMA_NAME),
          field.get(JdbcConstants.INTERNAL_TABLE_NAME),
  ...
  public JsonSchemaType getAirbyteType(SinglestoreType singlestoreType) {
    return switch (singlestoreType) {
      case TINYINT, TINYINT_UNSIGNED, SMALLINT, SMALLINT_UNSIGNED, MEDIUMINT, MEDIUMINT_UNSIGNED, INT, INT_UNSIGNED, BIGINT, BIGINT_UNSIGNED ->
          JsonSchemaType.INTEGER;
      case FLOAT, FLOAT_UNSIGNED, DOUBLE, DOUBLE_UNSIGNED, DECIMAL, DECIMAL_UNSIGNED ->
