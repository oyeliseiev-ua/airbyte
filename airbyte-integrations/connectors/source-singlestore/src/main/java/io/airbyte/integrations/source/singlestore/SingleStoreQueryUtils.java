...
public class SingleStoreQueryUtils {
  ...
  public static String getMaxPkValueForStream(final JdbcDatabase database, final ConfiguredAirbyteStream stream, final String pkFieldName,
  ...
  /**
   * Iterates through each stream and find the max cursor value and the record count which has that value based on each cursor field provided by the
   * customer per stream. This information is saved in a Hashmap with the mapping being the AirbyteStreamNameNamespacePair -> CursorBasedStatus.
   *
  ...
  public static Map<io.airbyte.protocol.models.v0.AirbyteStreamNameNamespacePair, CursorBasedStatus> getCursorBasedSyncStatusForStreams(
      final JdbcDatabase database, final List<ConfiguredAirbyteStream> streams, final StateManager stateManager, final String quoteString) {
